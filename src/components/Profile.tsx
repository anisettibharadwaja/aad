...
import { Trophy, Target, Zap, Clock, ArrowLeft, Award, Users, RefreshCw } from 'lucide-react';
import { cn } from '../lib/utils';
import { toast } from 'sonner';

export default function Profile({ user, profile: initialProfile, onBack }: { user: User; profile: UserProfile | null; onBack: () => void }) {
  ...
  if (loading) return <div className="flex items-center justify-center h-screen font-mono">RETRIEVING PROFILE DATA...</div>;
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-1 hardware-card p-8 flex flex-col items-center text-center space-y-4">
          ...
          <p className="text-[10px] opacity-50 font-mono">XP: {profile?.xp} / {(profile?.level || 1) * 100}</p>
        </div>
        ...
        <div className="md:col-span-2 grid grid-cols-2 gap-4">
          <div className="hardware-card p-6 flex flex-col justify-between">
            ...
            <div className="space-y-1">
              <div className="text-3xl font-bold font-mono">{profile?.wins}</div>
              <div className="text-[10px] uppercase opacity-50 font-mono tracking-widest">TOTAL WINS</div>
            </div>
          </div>
          <div className="hardware-card p-6 flex flex-col justify-between">
            <Target size={24} className="opacity-30" />
            <div className="space-y-1">
              <div className="text-3xl font-bold font-mono">{profile?.losses}</div>
          ...
          <div className="hardware-card p-6 flex flex-col justify-between">
            ...
            <div className="space-y-1">
              <div className="text-3xl font-bold font-mono">{profile?.xp}</div>
